...
def build_ortho(chunk, exportdir, doc, home, name, doc_title, b_ortho, e_ortho_lr, e_ortho_hr, r_ortho_lr, r_ortho_hr,
    ...
    compressionB = MS.ImageCompression()
    compressionB.tiff_compression = MS.ImageCompression.Tiff_big=True
    compressionC = MS.ImageCompression()
    compressionC.tiff_compression = MS.ImageCompression.Tiff_big=False
    ...
    if bt_ortho_lr == "TRUE":  # if low res ortho big tiff option true
        if e_ortho_lr == "TRUE":
            if chunk.orthomosaic is not None:
                ...
                chunk.exportRaster(ortho_path_lr, raster_transform=MS.RasterTransformNone, resolution_x=ortho_res_lr,
                                        resolution_y=ortho_res_lr, image_compression=compressionB,
                                        save_alpha=True)
            else:
        ...
        if e_ortho_lr == "TRUE":
            if chunk.orthomosaic is not None:
                ...
                ortho_path_lr = exportdir + "/" + doc_title + "_Ortho_LR_" + str(o_mm_lr) + "mm.tiff"
                chunk.exportRaster(ortho_path_lr, raster_transform=MS.RasterTransformNone, resolution_x=ortho_res_lr,
                                        resolution_y=ortho_res_lr, image_compression=compressionC,
                                        save_alpha=True)
            else:
                print ("must build Orthomosaic before export")
        else:
    ...
    if bt_ortho_hr == "TRUE": # high res big tiff option true
        if e_ortho_hr == "TRUE":
            if chunk.orthomosaic is not None:
                ...
                chunk.exportRaster(ortho_path_hr, raster_transform=MS.RasterTransformNone, resolution_x=ortho_res_hr,
                                        resolution_y=ortho_res_hr, image_compression=compressionB,
                                        save_alpha=True)
            else:
                print ("must build Orthomosaic before export")
        else:
        ...
        if e_ortho_hr == "TRUE":
            if chunk.orthomosaic is not None:
                ...
                ortho_path_hr = exportdir + "/" + doc_title + "_Ortho_HR_" + str(o_mm_hr) + "mm.tiff"
                chunk.exportRaster(ortho_path_hr, raster_transform=MS.RasterTransformNone, resolution_x=ortho_res_hr,
                                        resolution_y=ortho_res_hr, image_compression=compressionC,
                                        save_alpha=True)
            else:
                print ("must build Orthomosaic before export")
        else:
    ...
    if bt_dsm_lr == "TRUE":
        if e_dsm_lr == "TRUE":
            if chunk.elevation is not None:
                ...
                chunk.exportRaster(dsm_path_lr, raster_transform=MS.RasterTransformNone, resolution_x=dsm_res_lr,
                                resolution_y=dsm_res_lr, image_compression=compressionB)
            else:
        ...
        if e_dsm_lr == "TRUE":
            if chunk.elevation is not None:
                ...
                dsm_res_lr = float(r_dsm_lr)
                dsm_path_lr = exportdir + "/" + doc_title + "_DSM_LR_" + str(mm_lr) +"mm.tiff"
                chunk.exportRaster(dsm_path_lr, raster_transform=MS.RasterTransformNone, resolution_x=dsm_res_lr, resolution_y=dsm_res_lr,
                                image_compression=compressionC)
            else:
                print ("build DSM before trying to export")
        else:
            print("low resolution DSM export option not selected")
    if bt_dsm_hr == "TRUE":
        if e_dsm_hr == "TRUE":
            if chunk.elevation is not None:
                ...
                chunk.exportRaster(dsm_path_hr, raster_transform=MS.RasterTransformNone, resolution_x=dsm_res_hr,
                                resolution_y=dsm_res_hr, image_compression=compressionB)
            else:
                print ("build DSM before trying to export")
        else:
        ...
        if e_dsm_hr == "TRUE":
            if chunk.elevation is not None:
                ...
                dsm_res_hr = float(r_dsm_hr)
                dsm_path_hr = exportdir + "/" + doc_title + "_DSM_HR_" + str(mm_hr) +"mm.tiff"
                chunk.exportRaster(dsm_path_hr, raster_transform=MS.RasterTransformNone, resolution_x=dsm_res_hr, resolution_y=dsm_res_hr,
                                tiff_big=False)
            else:
                print ("build DSM before trying to export")
        else:
