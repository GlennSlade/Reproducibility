...
# ------ Written for PhotoScan 1.4.3 64 Revised for Metashape 1.6.4 October 2020------------------------
#---------Revised for Metashape 2.0 and RTK image data May 2023
#######################################################################################################################
...
def build_DPC(chunk, dpc_quality, pair_dm_lim, pair_dm_val, pair_dpc_lim, pair_dpc_val):
    ...
    outside_BB = len([p for p in chunk.tie_points.points if p.selected])
    print ("number of points outside bounding box is:" + str(outside_BB))

    n_points_final_SPC = len(chunk.tie_points.points) - outside_BB
    ...
    if pair_dpc_lim == "TRUE":  # This part determines if a pair limit is required for depth map filtering
        ...
        MS.app.settings.setValue('main/point_cloud_max_neighbors', float(pair_dpc_val))  # sets value based on input file
    else:
        MS.app.settings.setValue('main/point_cloud_max_neighbors', -1)
    ...
    #chunk.buildDenseCloud(point_colors=True) MS2.0 renames dense cloud as point cloud

    chunk.buildPointCloud(point_colors=True)

    # count number of points in dense cloud
    n_points_orig_DPC = chunk.point_cloud.point_count
