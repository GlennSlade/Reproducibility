...
def script_setup():
    ...
    dpc_npoints = chunk.point_cloud.point_count
...
def export_DPC(chunk, doc_title, exportdir, e_DPC):  # export points
    if e_DPC == "TRUE":
        if chunk.point_cloud is not None:
            ...
            dpc_name = "/LAZ/" + doc_title + "_dpc_export.laz"
            chunk.exportPointCloud(exportdir + dpc_name, crs=chunk.crs)  # specify projection to use. Precision attribute has been removesd in Metasahep
        else:
...
def build_mesh(chunk, doc, home, name, b_mesh, mesh_qual):
    ...
    if b_mesh == "TRUE":
        ...
        if mesh_qual == "LowFaceCount":
            chunk.buildModel(surface_type=MS.HeightField, source_data=MS.PointCloudData, interpolation= MS.EnabledInterpolation,
