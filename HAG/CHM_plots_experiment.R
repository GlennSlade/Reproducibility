...
for (i in 1:64) {
  ...
  #Import plot polygon
  P_sf <- read_sf(dsn = "/raid/home/gs558/share/Reproducibility/Plot/Plot_Polygon_Shape", layer = paste0("Plot_P1"))
  ...
  #Make DTM for each buffered plot     
       
 
  aoi_terra2 <- vect(P_buf)
  dtm_crop_buf <- crop(DTM_site, aoi_terra2)
  dtm_mask_buf <- mask(dtm_crop_buf, aoi_terra2)
  ...
  p <-as.points(dtm_mask_buf) |>sample()
  
  idw_dtm_buf <- terra::interpIDW(dtm_mask_buf, p, field="Z", radius=0.4, power=2, smooth=0, near=T)
  
  plot(idw_dtm_buf)
  
  #Make DTM for each plot 
  
  dtm <- rast (paste0 ("/raid/home/gs558/share/Reproducibility/Plot/DTM/P1_DTM.tif"))
  
  aoi_terra2 <- vect(P_buf)
  dtm_crop <- crop(dtm, aoi_terra2)
  dtm_mask <- mask(dtm_crop, aoi_terra2)
  
  #Calculate CHM - normalised Laz (buffered plot)

  nlas_buf <- Laz_buf - dtm_mask
