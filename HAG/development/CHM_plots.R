...
df <- read_xlsx("/raid/home/gs558/share/Reproducibility/Processed/plot_chm_metrics.xlsx")
...
for (i in 45:45) {
  ...
  # Account for the offset between RTK height and camera height to give true Canopy height
  chm2 <- chm-0.3003
  plot(chm)
...
#write_xlsx(df,"/raid/home/gs558/share/Reproducibility/Processed/plot_chm_metrics.xlsx")


p <-as.points(chm) |>sample()
#p  <- as.points(x)  |> sample()
idw4 <- terra::interpIDW(chm, p, field="Z", radius=0.1, power=2, smooth=0, near=T)
