...
df2 <- master_df%>% select (survey,plot,Mn_chm,Wind_Av,Sun_Elev_calc, Sun_Percent)

df3<-summarise(group_by(df2, survey),
               CHM=mean(Mn_chm),
               Wind=mean(Wind_Av),Sun = mean(Sun_Elev_calc), SunP = mean(Sun_Percent))              
...
P1 <- ggplot(df_temp) +
  ...
  theme_fancy() +
  #add title and labels
  ggtitle(paste0("Comparison of Sun Elevation and CHM mean values from surveys"))+
  #theme(aspect.ratio=1)+
