...
df2 <- df4 %>% dplyr::select (survey,plot,Mn_chm,Wind_Av,Sun_Elev_calc, Sun_Percent, empty_prop, PlotGenus.x,RDCHM,illumination,binary_skycode,CHM_MEAN)

# summarise by survey and plot genus (giving average canopy height for each species on each survey)

df3<-summarise(group_by(df2, survey,PlotGenus.x),
               CHM=mean(Mn_chm),
               Wind=mean(Wind_Av),Sun = mean(Sun_Elev_calc), SunP = mean(Sun_Percent), Empty = mean(empty_prop),RDCHM = mean(RDCHM))              
...
ggplot2::ggsave(
  ...
  filename = paste0("output_data/summary_wind_species_RDCHM.jpg"),
  width = 10,
  height = 10,
  units = "cm"
) 
...
mod1 <- lm(RDCHM~ Wind * PlotGenus.x,data = df)
summary(mod1)
P3 <-ggpredict(mod1, 
          terms = c("Wind","PlotGenus.x")) |>  plot()

P3

performance::check_model(mod1)  # Evaluate model performance
performance::r2(mod1)
summary(mod1)
ggplot2::ggsave(
  P3,
  # filename = "/plots/test.png",
  filename = paste0("output_data/summary_wind_species_ggeffect_RDCHM.jpg"),
  width = 16,
  height = 10,
  units = "cm"
) 
