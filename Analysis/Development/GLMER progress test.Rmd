...
## GLMER progressive test of possible models
...
df_wind <- df4 %>% dplyr::select (survey,plot,Mn_chm,Wind_Av,Sun_Elev_calc, Sun_Percent, empty_prop, PlotGenus.x,RDCHM,illumination,binary_skycode, CHM_MAX)
...
# 3. Fixed Effect Wind_Av and Sun Elevation (when sunny) Random effect Genus - filtered for sunny data only
...
#11. Fixed effect wind and MaxCHM (plant height proxy) with plot genus as random

wind_model11 <-lme4::glmer(RDCHM ~  Wind_Av + CHM_MAX + (1|PlotGenus.x),
                          data = df_wind,
                          family = Gamma(link = "identity"))

performance::check_model(wind_model11)  # Evaluate model performence
