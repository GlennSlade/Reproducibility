...
# NB. Ideally this global map would use the Winkel-Tripel projection to
# further minimize distortion, however this projection isn't yet
# supported in the SF package so for simplicity we've used the Robinson
# projection
...
# Calculate maximum XY ratio in the df.
{
  ...
  intervals <- c(0.5, 0.2, 0.2, 0.5, 0.4, 0.1)
...
### Extract model parameters for different species
{
...
## Also used to order the facet plot below!
{
...
### 6.2 Visualizing wind and sun influence ----
## Review distribution of environmental parameters
...
## Create histogram of solar elevation
...
### Create wind influence plot at PFT-level
{
...
### Create sun influence plot at PFT-level
{
...
performance::check_model(cloud_model)  # Evaluate model performance
...
# Visualize cloud interaction model ----
{
...
species_R2_median <- round(median(species_summary3$lm_r2),2)
...
species_adjR2_median <- round(median(species_summary3$lm_adjr2),2)
...
species_loocv_median <- round(median(species_summary3$lm_loocv_pc),1)
...
harvested_area <- round(sum(df_peak$plot_area_from_coordinates_m2),1)
...
cat("Drone Allometry Experiment: Summary Report \n",
    ...
    "In total, we harvested ", harvested_area, " m2.", "\n", "\n",
    "--------------------------------------------, -------------------------------------", "\n",
    "End of Report", "\n",
