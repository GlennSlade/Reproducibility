...
P1 <- ggplot(df_temp) +
  ...
  ggtitle(paste0("Comparison of Wind speed and RDCHM mean values from surveys"))+
...
ggplot2::ggsave(
  ...
  filename = paste0("output_data/summary_wind_RDCHM.jpg"),
  width = 10,
  height = 10,
  units = "cm"
) 
...
df7 <- df_B %>% dplyr::select (survey,plot,RDCHM,RDCHM_Percent,Mn_chm,Wind_Av,Sun_Elev_calc, Sun_Percent, empty_prop,Wind_SD,Wind_Max)
...
P1betula <- ggplot(df_temp) +
  ...
  theme_fancy() +
  #add title and labels
  ggtitle(paste0("Betula Plots \n Comparison of Wind speed and RDCHM mean values from surveys"))+
  #theme(aspect.ratio=1)+
  xlab('Wind Speed (m/s)')+
  ylab('RDCHM - Mean all plots')
#coord_equal(ratio=1)
...
df7 <- df_S %>% dplyr::select (survey,plot,RDCHM,RDCHM_Percent,Mn_chm,Wind_Av,Sun_Elev_calc, Sun_Percent, empty_prop,Wind_SD,Wind_Max)
...
df7 <- df_U %>% dplyr::select (survey,plot,RDCHM,RDCHM_Percent,Mn_chm,Wind_Av,Sun_Elev_calc, Sun_Percent, empty_prop,Wind_SD,Wind_Max)
...
df7 <- df_F %>% dplyr::select (survey,plot,RDCHM,RDCHM_Percent,Mn_chm,Wind_Av,Sun_Elev_calc, Sun_Percent, empty_prop,Wind_SD,Wind_Max)
