...
p14 <- ggplot(data = master_df, mapping = aes (x=survey, y=RDCHM, group = survey))+ 
  ...
  xlab("Survey Number") + ylab("Reconstructed Height - difference from mean measurement (m)")+theme_fancy()+ 
...
ggplot2::ggsave(
  ...
  filename = paste0("C:/Workspace/R_Scripts/Reproducibility/output_data/GCP_reconstructed_height_difference from mean.jpg"),
  width = 16,
  height = 8,
  units = "cm"
) 
...
p15 <- ggplot(data = master_df, mapping = aes (x=survey, y=RDGNSS, group = survey))+ 
  stat_boxplot(fill="steelblue",outlier.shape = NA)+
  xlab("Survey Number") + ylab("Reconstructed GCP Marker Elevation \n - difference from GNSS measurement (m)")+theme_fancy()+ 
  geom_hline(yintercept=0, linetype="dashed", color = "grey")+ geom_hline(yintercept=-0.0134, linetype="dashed", color = "steelblue")
plot (p15)
...
ggplot2::ggsave(
  p15,
  # filename = "/plots/test.png",
  filename = paste0("C:/Workspace/R_Scripts/Reproducibility/output_data/full_model/Figure S2 GCP_reconstructed_height_difference from GNSS.jpg"),
  width = 16,
  height = 8,
  units = "cm"
) 
...
df3<-summarise(group_by(master_df, survey),
               CHM=mean(Mn_chm),
               Wind=mean(Wind_Av),Mean_dif_GCP = mean(RDCHM),mean_dif_GNSS_GCP = mean(RDGNSS))              
